...
"""SO101 joint-position pick-place environment configuration.

Layout mirrors the OMX equivalent but swaps in SO101 assets and joint /
link names. End-effector and wrist camera are attached under the SO101
follower xform ``so101_follower/gripper``.
"""
...
from robotis_lab.real_world_tasks.manager_based.SO101.pick_place.mdp import so101_pick_place_events
from robotis_lab.real_world_tasks.manager_based.SO101.pick_place.pick_place_env_cfg import (
    PickPlaceEnvCfg,
    SO101_ALL_JOINTS,
)
...
# Default initial pose: 6 SO101 joints (5 arm + 1 gripper), all zero.
# Tune via env_cfg.json -> events.init_so101_arm_pose.default_pose.
_DEFAULT_EV = {
    "init_so101_arm_pose": {
        # Last entry (gripper) = math.radians(-10) so the gripper resets fully
        # closed, matching the -10 deg lower limit of the SO101 USD gripper joint.
        "default_pose": [-0.06828, -1.70466, 1.39779, 1.37708, -0.10203, -0.17453292519943295],
    },
    "randomize_so101_joint_state": {"mean": 0.0, "std": 0.02},
    "randomize_bottle_positions": {
        "pose_range": {
            "x": [-0.05, 0.04], "y": [0.2, 0.2],
            "z": [-0.30074, -0.30074],
    ...
    "randomize_basket_positions": {
        "pose_range": {
            "x": [-0.1848, -0.1848], "y": [0.27301, 0.27301],
            "z": [-0.29502, -0.29502],
            "roll": [-1.5707963267948966, -1.5707963267948966],
            "pitch": [3.141592653589793, 3.141592653589793],
            "yaw": [0.0, 0.0],
        },
        "min_separation": 0.1,
    },
...
_DEFAULT_SC = {
    "cam_wrist": {
        ...
        "spawn": {
            "focal_length": 2.79, "focus_distance": 200.0,
            "horizontal_aperture": 3.2, "clipping_range": [0.01, 100.0],
        },
        # NOTE: pose copied from OMX; tune for SO101 gripper geometry.
        "offset": {
            "pos": [0.0004, 0.02965, -0.03725],
            "rot": [0.98371, -0.17648, 0.01869, -0.02865],
        },
    },
    "cam_top": {
        "height": 480, "width": 640,
        "spawn": {
            "focal_length": 16.9157, "focus_distance": 102.3,
            "horizontal_aperture": 20.955, "clipping_range": [0.01, 100.0],
        },
        "offset": {
            "pos": [-0.0203, -0.1053, 0.4543],
            "rot": {
                "roll": 23.278100642635437,
                "pitch": 0.8467805711243336,
                "yaw": -1.0833502299705997,
            },
        },
    },
}
...
class EventCfg:
    ...
    init_so101_arm_pose = EventTerm(
        func=so101_pick_place_events.set_default_joint_pose,
        mode="reset",
        params={
            "default_pose": _ev["init_so101_arm_pose"]["default_pose"],
        },
    )

    randomize_so101_joint_state = EventTerm(
        func=so101_pick_place_events.randomize_joint_by_gaussian_offset,
        mode="reset",
        params={
            "mean": _ev["randomize_so101_joint_state"]["mean"],
            "std": _ev["randomize_so101_joint_state"]["std"],
            "asset_cfg": SceneEntityCfg("robot"),
        },
    )

    randomize_bottle_positions = EventTerm(
        func=so101_pick_place_events.randomize_object_pose,
        mode="reset",
        params={
    ...
    randomize_scene_light = EventTerm(
        func=so101_pick_place_events.randomize_scene_lighting_domelight,
        mode="reset",
        params={
    ...
    randomize_sphere_light = EventTerm(
        func=so101_pick_place_events.randomize_scene_lighting_spherelight,
        mode="reset",
        params={
    ...
    randomize_top_camera = EventTerm(
        func=so101_pick_place_events.randomize_camera_pose,
        mode="reset",
        params={
...
class SO101BottlePickPlaceEnvCfg(PickPlaceEnvCfg):
    def __post_init__(self):
        ...
        # sanity-check: ensure default_pose length matches joint count
        if len(_ev["init_so101_arm_pose"]["default_pose"]) != len(SO101_ALL_JOINTS):
            print(
                f"[WARN] init_so101_arm_pose.default_pose has "
                f"{len(_ev['init_so101_arm_pose']['default_pose'])} entries; "
                f"expected {len(SO101_ALL_JOINTS)} (one per joint)"
            )
        ...
        # Reuse OMY table (top camera mounts under camera_link)
        ...
        # Wrist camera mounted on the SO101 gripper link.
        self.scene.cam_wrist = CameraCfg(
            prim_path="{ENV_REGEX_NS}/Robot/so101_follower/gripper/cam_wrist",
            update_period=0.0,
        ...
        # Top camera mounted on the table fixture.
        ...
        # End-effector frame: target the SO101 gripper link.
        marker_cfg = FRAME_MARKER_CFG.copy()
        marker_cfg.markers["frame"].scale = (0.1, 0.1, 0.1)
        marker_cfg.prim_path = "/Visuals/FrameTransformer"
        self.scene.ee_frame = FrameTransformerCfg(
            prim_path="{ENV_REGEX_NS}/Robot/so101_follower/base",
            debug_vis=False,
            visualizer_cfg=marker_cfg,
            target_frames=[
                FrameTransformerCfg.FrameCfg(
                    prim_path="{ENV_REGEX_NS}/Robot/so101_follower/gripper",
                    name="end_effector",
                    offset=OffsetCfg(
                        pos=[0.0, 0.0, -0.09],
                    ),
