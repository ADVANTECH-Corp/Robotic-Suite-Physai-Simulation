...
def _merge_with_log(defaults: dict, loaded: dict, section: str) -> dict:
    merged = {}
    for key, default_val in defaults.items():
        if key in loaded:
            if isinstance(default_val, dict) and isinstance(loaded[key], dict):
                merged[key] = _merge_with_log(default_val, loaded[key], f"{section}.{key}")
            else:
                merged[key] = loaded[key]
                print(f"[env_cfg] {section}.{key} = loaded from JSON: {loaded[key]}")
        else:
            merged[key] = default_val
            print(f"[env_cfg] {section}.{key} = using DEFAULT: {default_val}")
    for key in loaded:
        if key not in defaults:
            merged[key] = loaded[key]
            print(f"[env_cfg] {section}.{key} = loaded from JSON (unknown key): {loaded[key]}")
    return merged
...
_DEFAULT_AC = {
    "record": {
        "arm_action": {
            "scale": 1.0,
            "offset": {"joint1": 0.0, "joint2": 0.02572, "joint3": 0.15, "joint4": 0.0015, "joint5": 0.0},
        },
        "gripper_action": {"scale": 1.0},
    },
    "mimic_ik": {
        "arm_action": {
            "controller": {"ik_params": {"lambda_val": 0.05}},
            "body_offset": {"pos": [0.09, 0.0, 0.0]},
        },
        "gripper_action": {"scale": 1.0},
    },
}
...
class ObjectTableSceneCfg(InteractiveSceneCfg):
    """Configuration for the pick and place scene with a robot and objects."""
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        joint_pos = ObsTerm(
            ...
            params={"joint_names": ["joint1", "joint2", "joint3", "joint4", "joint5", "gripper_joint_1"],
                    "asset_name": "robot"},
        )
        joint_vel = ObsTerm(
...
class TerminationsCfg:
    ...
    bottle_dropping = DoneTerm(
        func=mdp.root_height_below_minimum, params={"minimum_height": -0.4, "asset_cfg": SceneEntityCfg("bottle")}
    )
