...
def randomize_scene_lighting_spherelight(
    env: ManagerBasedEnv,
    env_ids: torch.Tensor,
    intensity_range: tuple[float, float] = (1000.0, 3000.0),
    color_range: tuple[tuple[float, float], tuple[float, float], tuple[float, float]] = ((0.5, 1.0), (0.5, 1.0), (0.5, 1.0)),
    pose_range: dict[str, tuple[float, float]] = None,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("SphereLight"),
):
    asset: AssetBase = env.scene[asset_cfg.name]
    
    for light_prim in asset.prims:
        # Random intensity
        new_intensity = random.uniform(intensity_range[0], intensity_range[1])
        intensity_attr = light_prim.GetAttribute("inputs:intensity")
        ...
        if pose_range is not None:
            new_pos = Gf.Vec3d(
                random.uniform(pose_range.get("x", (0.0, 0.0))[0], pose_range.get("x", (0.0, 0.0))[1]),
                random.uniform(pose_range.get("y", (0.0, 0.0))[0], pose_range.get("y", (0.0, 0.0))[1]),
                random.uniform(pose_range.get("z", (0.0, 0.0))[0], pose_range.get("z", (0.0, 0.0))[1]),
            )
