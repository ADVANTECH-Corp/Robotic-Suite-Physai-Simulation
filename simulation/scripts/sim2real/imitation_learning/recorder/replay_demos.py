...
parser.add_argument("--publish_dds", action="store_true", default=False, help="Publish actions to DDS topic /leader/joint_trajectory.")
parser.add_argument("--robot_type", type=str, default="OMX", choices=["OMX", "SO101"], help="Robot type for DDS joint names.")
...
# DDS joint names per robot type.
# Order must match the action tensor dimension order produced by each env's
# ActionsCfg (arm_action joints first, then gripper_action).
DDS_JOINT_NAMES = {
...
def create_dds_publisher(domain_id: int = 0):
    """Create a DDS topic writer for /leader/joint_trajectory."""
...
def publish_action_to_dds(writer, joint_names: list, action_tensor: torch.Tensor):
    """Publish an action tensor as a JointTrajectory_ message."""
    ...
    positions = action_tensor.squeeze(0).cpu().tolist()
    # Only take as many positions as we have joint names
    ...
    point = JointTrajectoryPoint_(
        ...
        accelerations=[],
        effort=[],
        time_from_start=Duration_(sec=0, nanosec=int(0.02 * 1e9)),
    )
    header = Header_(stamp=Time_(sec=0, nanosec=0), frame_id="")
...
def main():
    ...
    # Initialize action configuration for replay mode
    ...
    # Setup DDS publisher if requested
    dds_writer = None
    dds_joint_names = None
    if args_cli.publish_dds:
        domain_id = int(os.getenv("ROS_DOMAIN_ID", 0))
        dds_writer = create_dds_publisher(domain_id)
        dds_joint_names = DDS_JOINT_NAMES[args_cli.robot_type]
        print(f"[DDS] Publishing actions to /leader/joint_trajectory (joints: {dds_joint_names})")
            ...
            while has_next_action:
                ...
                if first_loop:
                    ...
                    while is_paused:
                        ...
                        continue
                env.step(actions)
                # Publish actions to DDS if enabled
                if dds_writer is not None:
                    publish_action_to_dds(dds_writer, dds_joint_names, actions)
