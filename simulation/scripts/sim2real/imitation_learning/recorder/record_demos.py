...
parser.add_argument("--robot_type", type=str, default="keyboard", choices=['OMX', 'SO101'], help="Type of robot to use for teleoperation.")
...
def main():
    ...
    if args_cli.robot_type == "OMX":
        ...
        teleop_interface = OMXSdk(env, mode='record')
    elif args_cli.robot_type == "SO101":
        from dds_sdk.so101_sdk import SO101Sdk
        teleop_interface = SO101Sdk(env, mode='record')
    else:
        raise ValueError(
        ...
        with torch.inference_mode():
            ...
            if should_reset_task_success:
                ...
                if env.recorder_manager.exported_successful_episode_count > current_recorded_demo_count:
                    ...
                    # Print num_samples for the recorded episode
                    try:
                        import h5py
                        with h5py.File(args_cli.dataset_file, "r") as f:
                            demo_name = f"demo_{current_recorded_demo_count - 1}"
                            demo = f["data"][demo_name]
                            print(f"  num_samples: {demo.attrs['num_samples']}")
                            print(f"  actions shape: {demo['actions'].shape}")
                    except Exception as e:
                        print(f"  Warning: Failed to read num_samples: {e}")
            ...
            # apply actions
                ...
                if isinstance(actions, dict):
                    ...
                    # t3 = time.time()
                    # step_count += 1
                    # # Timing: print detailed breakdown
                    # loop_end = time.time()
                    # pub_ms = (t1 - t0) * 1000
                    # get_ms = (t2 - t1) * 1000
                    # step_ms = (t3 - t2) * 1000
                    # total_ms = (loop_end - loop_start) * 1000
                    # print(f"[Timing] pub:{pub_ms:.0f}ms get:{get_ms:.0f}ms step:{step_ms:.0f}ms total:{total_ms:.0f}ms | Steps:{step_count}")

            # Apply rate limiter; during recording only sleep (skip render to avoid overhead)
            if rate_limiter:
                if start_record_state:
                    sleep_time = rate_limiter.last_time + rate_limiter.sleep_duration - time.time()
                    if sleep_time > 0:
                        time.sleep(sleep_time)
                    rate_limiter.last_time += rate_limiter.sleep_duration
                    if rate_limiter.last_time < time.time():
