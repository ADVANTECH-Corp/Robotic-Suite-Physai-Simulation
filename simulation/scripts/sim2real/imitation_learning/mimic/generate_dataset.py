...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Skip exporting the generated dataset to HDF5 file.",
)
parser.add_argument(
    "--seed",
    type=int,
    default=-1,
    help="Random seed for data generation. -1 means random seed each run.",
)
...
def main():
    ...
    if args_cli.seed >= 0:
        env.cfg.datagen_config.seed = args_cli.seed
    else:
        import time
        env.cfg.datagen_config.seed = int(time.time()) % 100000
    print(f"[Seed] Using seed: {env.cfg.datagen_config.seed}")
