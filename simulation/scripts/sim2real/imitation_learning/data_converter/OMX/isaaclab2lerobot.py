...
def process_data(
    ...
    # Correct gripper clamp artifact: omy_sdk clamps gripper_joint_1 to minimum 0.12 during
    # recording, so values <= 0.12 in the HDF5 represent "fully closed" and should be 0.
    GRIPPER_IDX = 5
    GRIPPER_MIN = 0.121
    actions[:, GRIPPER_IDX] = np.where(actions[:, GRIPPER_IDX] <= GRIPPER_MIN, 0.0, actions[:, GRIPPER_IDX])
    joint_pos[:, GRIPPER_IDX] = np.where(joint_pos[:, GRIPPER_IDX] <= GRIPPER_MIN, 0.0, joint_pos[:, GRIPPER_IDX])
