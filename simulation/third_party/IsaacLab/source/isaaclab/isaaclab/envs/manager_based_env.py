...
class ManagerBasedEnv:
    ...
    * **Recorder Manager**: The recorder manager that handles recording data produced during different steps
    ...
    def __init__(self, cfg: ManagerBasedEnvCfg):
        ...
        if builtins.ISAAC_LAUNCHED_FROM_TERMINAL is False:
            ...
            with Timer("[INFO]: Time taken for simulation start", "simulation_start"):
                ...
                # Cameras may still return empty RGB after the default 2 warmup renders in
                # SimulationContext.reset(). ObservationManager locks term shapes on first
                # sample, so wait until RGB frames match configured HxW before load_managers.
                self._wait_for_camera_rgb_frames()
    ...
    def _wait_for_camera_rgb_frames(self, max_extra_renders: int = 600) -> None:
        """Render until RGB/RGBA camera sensors produce frames matching their configured resolution.

        ``SimulationContext.reset()`` only performs two warmup renders. With livestream / cold GPU
        paths those frames can still be empty, and ``ObservationManager`` then locks term shapes to
        ``(0,)``. Extra render+update cycles here prevent that until cameras are ready or we timeout.

        Camera buffers may first be allocated from empty annotator frames (shape size 0). When real
        ``(H, W, C)`` frames arrive later, assignment into that buffer raises ``RuntimeError: expand``.
        Clear invalid buffers so the next update reallocates from the current annotator output.
        """
        camera_sensors: list[tuple[str, Any]] = []
        for name, sensor in self.scene.sensors.items():
            data_types = getattr(getattr(sensor, "cfg", None), "data_types", None)
            if not data_types:
                continue
            if "rgb" not in data_types and "rgba" not in data_types:
                continue
            if not hasattr(sensor, "image_shape"):
                continue
            camera_sensors.append((name, sensor))

        if not camera_sensors:
            return

        print(
            f"[INFO]: Waiting for {len(camera_sensors)} camera sensor(s) to produce non-empty RGB "
            f"(max extra renders: {max_extra_renders})..."
        )

        def _image_key_and_tensor(output: dict) -> tuple[str | None, Any]:
            key = "rgb" if "rgb" in output else ("rgba" if "rgba" in output else None)
            if key is None:
                return None, None
            return key, output.get(key)

        def _sensor_ready(sensor: Any) -> bool:
            # Accessing the ``.data`` property (not the raw ``_data`` attribute) is required:
            # it is what actually triggers ``_update_outdated_buffers()`` when the sensor is
            # marked outdated. Reading ``_data`` directly would only ever see stale/empty state.
            output = sensor.data.output
            if not output:
                return False
            key, img = _image_key_and_tensor(output)
            if key is None or img is None or not hasattr(img, "numel") or img.numel() == 0:
                return False
            height, width = sensor.image_shape
            channels = 3 if key == "rgb" else 4
            # Expected layout: (num_envs, H, W, C)
            if len(img.shape) < 3:
                return False
            return (
                int(img.shape[-3]) == int(height)
                and int(img.shape[-2]) == int(width)
                and int(img.shape[-1]) >= channels
            )

        # Shape-mismatch errors that mean "not ready yet", not a fatal bug:
        # - "expand(...)": an in-place assign into a buffer previously locked at size=0
        #   by an earlier empty frame, now receiving a real (H, W, C) frame.
        # - "stack expects each tensor to be equal size": with num_envs > 1, each env's
        #   camera can become ready at a different render, so torch.stack() across envs
        #   sees a mix of empty and valid frames while (re)building the buffer.
        _CAMERA_SHAPE_MISMATCH_MARKERS = ("expand(", "stack expects each tensor to be equal size")

        def _force_camera_refresh(sensor: Any) -> None:
            # ``InteractiveScene.update()`` calls ``sensor.update(dt, force_recompute=False)``
            # whenever ``cfg.lazy_sensor_update`` is True (the default), which skips the actual
            # buffer refresh entirely. Force it here so every render actually pulls a new frame
            # from the annotator instead of leaving stale/empty data in place.
            is_outdated = getattr(sensor, "_is_outdated", None)
            if is_outdated is not None:
                is_outdated[:] = True
            try:
                sensor.update(self.physics_dt, force_recompute=True)
            except RuntimeError as err:
                if not any(marker in str(err) for marker in _CAMERA_SHAPE_MISMATCH_MARKERS):
                    raise
                # Drop the (possibly partially-built) buffer and mark outdated again so the
                # *next* render/refresh cycle rebuilds it from scratch. Do not retry inline
                # here: a second mismatch (e.g. different envs still out of sync) would then
                # propagate uncaught and crash environment creation.
        ...
        for i in range(max_extra_renders + 1):
            if all(_sensor_ready(sensor) for _, sensor in camera_sensors):
                print(f"[INFO]: Camera sensors ready after {i} extra render(s).")
                return
            if i == max_extra_renders:
                break
            self.sim.render()
            for _, sensor in camera_sensors:
                _force_camera_refresh(sensor)

        details = []
        for name, sensor in camera_sensors:
            output = getattr(getattr(sensor, "_data", None), "output", None) or {}
            key, img = _image_key_and_tensor(output)
            shape = None if img is None else getattr(img, "shape", None)
            details.append(f"{name}={shape}")
        raise RuntimeError(
            "Camera sensors did not produce valid RGB frames before ObservationManager init: "
            + ", ".join(details)
        )
